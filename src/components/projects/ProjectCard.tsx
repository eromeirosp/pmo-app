...
import { User, Calendar, DollarSign, Building2, CheckCircle2, AlertTriangle, XCircle, TrendingUp, Activity } from "lucide-react";
...
export function ProjectCard({ project, healthScore, healthTrend, alertCount }: ProjectCardProps) {
    const badgeColor = statusColorMap[project.status as keyof typeof statusColorMap] || statusColorMap.GREEN;

    const formattedBudget = formatCurrency(project.budget, project.currency || "BRL");
    ...
    return (
        <Link href={`/projects/${project.id}`} className="block group h-full">
            <div className={`h-full flex flex-col p-6 bg-card border border-border rounded-2xl shadow-md transition-all duration-300 hover:border-primary/30 hover:shadow-xl hover:shadow-primary/5 hover:-translate-y-1 relative overflow-hidden ${isClosed ? "opacity-75" : ""}`}>
                {/* Accent Gradient */}
                ...
                <div className="flex items-start justify-between gap-2 mb-3">
                    <h4 className="text-[18px] font-bold tracking-tight text-foreground group-hover:text-primary transition-colors line-clamp-2 leading-tight">
                        {project.name}
                    </h4>
                    {healthScore != null && (
                        <div className="flex items-center gap-1 shrink-0" title={`Health Score: ${healthScore}/100`}>
                            <Activity className={`h-3.5 w-3.5 ${healthScore >= 80 ? 'text-emerald-500' : healthScore >= 50 ? 'text-amber-500' : 'text-rose-500'}`} />
                            <span className={`text-[12px] font-black ${healthScore >= 80 ? 'text-emerald-500' : healthScore >= 50 ? 'text-amber-500' : 'text-rose-500'}`}>
                                {healthScore}
                            </span>
                        </div>
                    )}
